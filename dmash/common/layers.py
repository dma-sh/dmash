...
class TransformerEncoder(Module):
    ...
    def forward(
        ...
        if convert_to_nested:
            output[0] = output[0].to_padded_tensor(0.0, src.size())
...
class TransformerEncoderLayer_hopfield(nn.Module):
    """
    ...
    def __init__(self,
        ...
        :param self_attn: instance of Hopfield association module
        ...
        """
        super(TransformerEncoderLayer_hopfield, self).__init__()
        self.self_attn = copy.deepcopy(hopfield_association)

        self.linear_residual = nn.Linear(self.self_attn.state_pattern_dim, dim_feedforward)
        self.dropout_residual = nn.Dropout(dropout)
        self.linear_output = nn.Linear(dim_feedforward, self.self_attn.state_pattern_dim)

        self.norm_residual = nn.LayerNorm(self.self_attn.state_pattern_dim)
        self.norm_output = nn.LayerNorm(self.self_attn.state_pattern_dim)
    ...
    def reset_parameters(self) -> None:
        ...
        :return: None
        """
        for module in (self.self_attn, self.linear_residual,
    ...
    def forward(self, src: Tensor, src_mask: Optional[Tensor] = None,
        ...
        return self.norm_output(input=src), None
    ...
    def get_association_matrix(self, input: Union[Tensor, Tuple[Tensor, Tensor, Tensor]]) -> Tensor:
        ...
        """
        return self.self_attn.get_association_matrix(input=input)
    ...
    def input_size(self) -> int:
        return self.self_attn.input_size
...
class PositionalEncoding(nn.Module):
    ...
    Args:
        ...
        batch_first: if True, input and output are of shape [batch size, sequence length, embed dim]
        instead of [sequence length, batch size, embed dim]
    Examples:
    ...
    def forward(self, x):
        ...
        Shape:
            depends on whether batch_first is True
            x: [sequence length, batch size, embed dim] or [batch size, sequence length, embed dim]
            output: [sequence length, batch size, embed dim] or [batch size, sequence length, embed dim]
        Examples:
            >>> output = pos_encoder(x)
        """
        if self.batch_first:
            x_perm = x.permute(1, 0, 2)
            penc_perm = self.pe[:x_perm.size(0), :]
            penc = penc_perm.permute(1, 0, 2)
        else:
            penc = self.pe[:x.size(0), :]
