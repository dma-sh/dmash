...
def update_physics(xml_string, pole_index=1, damping=2.e-6, length=0.5, mass=0.1, action_factor=1.0, length_factor=1.0):
    """
    ...
    # Update joint damping for the spinner
    bodies = mjcf.findall("./worldbody/body/body")
    for body in bodies:
        if body.get("name") == f"pole_{pole_index}":
            joints = body.findall(".//joint")
            for joint in joints:
                if joint.get("name") == f"hinge_{pole_index}":
                    joint.set("damping", str(damping))
            geoms = body.findall(".//geom")
            for geom in geoms:
                if geom.get("name") == f"pole_{pole_index}":
                    geom.set("mass", str(mass))
                    geom.set("fromto", " ".join(["0", "0", "0", "0", "0", str(abs(2 * length))]))
...
def balance(time_limit=_DEFAULT_TIME_LIMIT, random=None,
            environment_kwargs=None, context_kwargs=None):
    ...
    if context_kwargs is not None:
        ...
        negative_length = (context_kwargs.get("length", 0.5) < 0.0) or (context_kwargs.get("length_factor", 1.0) == -1.0)
    ...
    task = Balance(swing_up=False, sparse=False, random=random, action_factor=action_factor, negative_length=negative_length)
...
def balance_sparse(time_limit=_DEFAULT_TIME_LIMIT, random=None,
    ...
    if context_kwargs is not None:
        ...
        negative_length = False
    physics = Physics.from_xml_string(xml_string, assets)
    task = Balance(swing_up=False, sparse=True, random=random, action_factor=action_factor, negative_length=negative_length)
    environment_kwargs = environment_kwargs or {}
    return control.Environment(
        physics, task, time_limit=time_limit, **environment_kwargs)
...
class Balance(base.Task):
    ...
    def __init__(self, swing_up, sparse, random=None, action_factor=None, negative_length=False):
    ...
    def initialize_episode(self, physics):
        ...
        if self._swing_up:
            ...
            upside_down = np.pi if self.negative_length else 0.0
            physics.named.data.qpos[1:] = self.random.uniform(-.034, .034, nv - 1) + upside_down
