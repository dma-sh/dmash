# https://github.com/ubisoft/ubisoft-laforge-disentanglement-metrics
# J. Zaidi, J. Boilard, G. Gagnon and M.A. Carbonneau,
# "Measuring Disentanglement: A Review of Metrics", arXiv:2012.09276, 2020.
...
def dci(factors, codes, continuous_factors=True, model='lasso'):
    ...
    assert continuous_factors, 'Only continuous factors are supported'
    ...
    for f in range(nb_factors):
        if np.sum(e_matrix[f, :]) != 0:
            ...
            prob = np.ones((len(e_matrix[f, :]))) / len(e_matrix[f, :])
...
def _fit_lasso(factors, codes):
    ...
    for a in alphas:
        ...
        clf = Lasso(alpha=a, max_iter=5000, random_state=0)
    ...
    clf = Lasso(alpha=best_a, random_state=0)
...
def _fit_random_forest(factors, codes):
    ...
    for md in max_depth:
        for mf in max_features:
            # perform cross validation on the tree classifiers
            clf = RandomForestRegressor(n_estimators=10, max_depth=md, max_features=mf, random_state=0)
            mse = cross_val_score(clf, codes, factors, cv=10, scoring='neg_mean_squared_error')
            mse = -mse.mean()
    ...
    # train the model using the best performing parameter
    clf = RandomForestRegressor(n_estimators=10, max_depth=best_md, max_features=best_mf, random_state=0)
    clf.fit(codes, factors)
