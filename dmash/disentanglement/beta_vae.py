...
def beta_vae(factors, codes, continuous_factors=True, nb_bins=10, batch_size=200,
    ...
    if train_set is np.nan and eval_set is np.nan:
        return np.nan

    # train model
    inputs, targets = train_set
    model = linear_model.LogisticRegression(max_iter=nb_max_iterations, random_state=0)
...
def _prepare_datasets(factors, codes, batch_size, nb_training, nb_eval):
    ...
    for factor_id in range(nb_factors):
        ...
        if factor_id not in training_factors:
            training_factors[factor_id] = 0
        if factor_id not in eval_factors:
        ...
        available_factor_values = [
            value for value in available_factor_values if available_factor_values[value] > 1
        ]
